...
* **시연 영상 핵심 시나리오 (3분 킬체인 라이브 데모)**:
  ...
  * **02:11 ~ 03:00 [AI 자동 대응]**: Google Gemini 3.6 / 3.5 Flash (Tier 1)가 침해 사고 5줄 요약 브리핑 및 MITRE ATT&CK 매핑 ➔ 관제사 **1-Click 방화벽 차단 & 단말 격리 플레이북** 실행
...
* **발표 자료 12장 슬라이드 구성안 (Pitch Deck Structure)**:
  ...
  4. **시스템 아키텍처 (Architecture)**: 수집(Agent/Tailscale) ➔ 정규화(Unified JSON) ➔ 엔진(상관분석) ➔ 대시보드(Streamlit+Gemini)
  5. **핵심 기술 ① 실시간 이기종 수집**: MySQL 2.4만 건 감사 API(`/since`) 및 Windows/Chrome 감사 에이전트
  6. **핵심 기술 ② 2단계 상관분석 모델**: NORMAL ➔ WATCH (선제 감시 68점) ➔ HIGH (유출 확정 95점) ➔ Self-Healing (자동 치유)
  7. **핵심 기술 ③ AI 기반 SOAR 연동**: Gemini 3.6 / 3.5 Flash (Tier 1) 브리핑, MITRE ATT&CK 매핑, 1-Click Zero Trust 격리
...
* **배포 URL**: 
  ...
  * 사내 DB 감사 연동 터널: `https://desktop-oli.tail2bbbea.ts.net/api/db-logs/since`
* **주요 기능**:
...
### 📑 기술 문서
* **API 문서**: `https://github.com/GIGANG-Team/GIGANG/blob/main/docs/API_SPECIFICATION.md`
  * `GET /api/db-logs/since?after={ISO_TIMESTAMP}&limit=100`: DB 감사 로그 증분 수집 API (X-API-Key 헤더 인증)
  * `POST /api/collect`: 단말 에이전트의 Unified JSON 로그 수집 웹훅
  * `GET /api/incidents/live`: 실시간 인시던트 상태 머신 모니터링 스트림
* **설치/실행 가이드**: `https://github.com/GIGANG-Team/GIGANG/blob/main/README.md#quick-start`
  * Python 3.10+ 가상환경 세팅 및 `pip install -r requirements.txt`
  * 환경변수 파일 `.env` 구성 (`GEMINI_API_KEY`, `DB_TUNNEL_URL`)
  * `streamlit run app.py`를 통한 올인원 관제 대시보드 구동
* **기술 스택**:
  * **Frontend / UI**: Streamlit, Plotly Express, Streamlit-AgGrid, Custom CSS3
  * **Backend / Engine**: Python 3.10+, FastAPI, SQLite3, Watchdog (파일 감시 엔진)
  * **Agent & Collector**: Windows C#/PowerShell 에이전트, Chrome Extension (Manifest v3)
  * **AI & LLM**: Google Gemini 3.6 Flash / 3.5 Flash Lite (Tier 1 고속 추론 인프라), MITRE ATT&CK v14 Knowledge Base
  * **Database & Infra**: MySQL 8.0 (`customer_vault` 24,500 rows), Tailscale Funnel, Railway Cloud

---

### 🎨 디자인 문서
* **UI/UX 디자인**: 
  * [피그마 링크 / 대시보드 와이어프레임 설계안]
  * **설계 컨셉**: SOC 보안 관제 최적화 다크 테마 (`#0E1117`), 3단 반응형 그리드 레이아웃 (KPI 요약 ➔ 킬체인 타임라인 ➔ Gemini AI 심층 분석 & 대응 패널)
* **브랜딩 가이드**:
...
* **팀원 구성 및 R&R**:
  * **강혜성 (팀장 / Dashboard & PM)**: 대시보드 UI/UX 구현, 시스템 통합, 일정 관리 및 라이브 데모 총괄
  ...
  * **팀원 3 (Engine & AI)**: 2단계 상태 머신 상관분석 엔진 구현 및 Gemini 3.6 Flash SOAR 플레이북 연동
