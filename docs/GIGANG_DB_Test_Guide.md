...
3. [GIGANG DB 조회 탐지 및 상관분석 메커니즘](#3-gigang-db-조회-탐지-및-상관분석-메커니즘)
...
GIGANG 상관분석 엔진(`gigang/engine/correlation.py`)은 다음 로직에 따라 DB 이벤트를 처리합니다.
...
1. Streamlit 앱 구동:
   ```bash
   streamlit run gigang/ui/app.py
   ```
